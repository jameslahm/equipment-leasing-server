...
class APITestCase(unittest.TestCase):
    ...
    def test_get_logs(self):
        response = self.client.get(
            '/api/logs', headers=self.get_api_headers(self.test_user))

        self.assertEqual(response.status_code, 401)

        type = ""
        response = self.client.get(
            '/api/logs?type={}&&page={}&&page_size={}&&start_time={}&&end_time={}'.format(type, 1, 10,0,1), headers=self.get_api_headers(self.admin))
